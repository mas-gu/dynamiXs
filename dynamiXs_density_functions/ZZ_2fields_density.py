...
class DualFieldSpectralDensityAnalysis:
    ...
    def propagate_dual_field_errors(self, r1_f1, r1_f1_err, r2_f1, r2_f1_err, noe_f1, noe_f1_err,
        ...
        # J(0) field 1 (Farrow-exact notation with d²/4)
        ...
        # J(0) field 2 (Farrow-exact notation with d²/4)
...
########################    ########################    ########################  
        ...
        # J(ωN) error for field 1 (Farrow-exact notation with d²/4)
        ...
        # J(ωH) error for field 1 (Farrow-exact notation with factor 4)
        ...
        # J(ωN) error for field 2 (Farrow-exact notation with d²/4)
        factor_jwn_f2 = 1.0 / (3.0 * d2 / 4.0 + c2)
        djwn_dr1_f2 = factor_jwn_f2 * (1.0 - (7.0/5.0) * (noe_f2 - 1.0) * gamma_ratio)  # Should be dr1, not dr2
        djwn_dnoe_f2 = factor_jwn_f2 * (-(7.0/5.0) * r1_f2 * gamma_ratio)
        jwn_f2_err = np.sqrt((djwn_dr1_f2 * r1_f2_err)**2 + (djwn_dnoe_f2 * noe_f2_err)**2)  # Should be r1_f2_err and noe_f2_err

        # J(ωH) error for field 2 (Farrow-exact notation with factor 4)
        factor_jwh_f2 = 4.0 / (5.0 * d2)
        djwh_dr1_f2 = factor_jwh_f2 * (noe_f2 - 1.0) * gamma_ratio  # Should be dr1, not dr2
        djwh_dnoe_f2 = factor_jwh_f2 * r1_f2 * gamma_ratio
        jwh_f2_err = np.sqrt((djwh_dr1_f2 * r1_f2_err)**2 + (djwh_dnoe_f2 * noe_f2_err)**2)  # Should be dr1_f2, not dr2_f2
...
########################    ########################    ########################    
        ...
        return {
            'Field1': {
                ...
                #'JwN': j_f1['JwN'], 'JwN_err': sigma_noe_f1_err * 0.5,  # Simplified
                #'JwH': j_f1['JwH'], 'JwH_err': sigma_noe_f1_err * 0.3   # Simplified
                'JwN': j_f1['JwN'], 'JwN_err': jwn_f1_err,  # Simplified
                'JwH': j_f1['JwH'], 'JwH_err': jwh_f1_err   # Simplified                          
            },
            'Field2': {
                'J0': j_f2['J0'], 'J0_err': j0_f2_err,
                #'JwN': j_f2['JwN'], 'JwN_err': sigma_noe_f2_err * 0.5,  # Simplified
                #'JwH': j_f2['JwH'], 'JwH_err': sigma_noe_f2_err * 0.3   # Simplified              
                'JwN': j_f2['JwN'], 'JwN_err': jwn_f2_err,  # Simplified
                'JwH': j_f2['JwH'], 'JwH_err': jwh_f2_err   # Simplified
            }
        }
    ...
    def plot_dual_field_results(self, results_df, save_plots=True):
        ...
        # Rex comparison
        axes[2,2].errorbar(residues, good_data['Rex_field1'], yerr=good_data['Rex_field1_err'],
                                          fmt='o', capsize=3, label=f'Rex {self.field1_freq} MHz', alpha=0.7)
        axes[2,2].errorbar(residues, good_data['Rex_field2'], yerr=good_data['Rex_field2_err'],
        ...
        if save_plots:
                plt.savefig('dual_field_rsdm_analysis_results.pdf', dpi=300, bbox_inches='tight')
                print("Plots saved as 'dual_field_rsdm_analysis_results.pdf'")
...
def main():
    """
    Example usage of the DualFieldSpectralDensityAnalysis class
    """
    ...
    try:
        ...
        # Display results
        print("\nDual-Field Reduced Spectral Density Analysis Results:")
        print("=" * 80)
        ...
        # Save basic results
        results.to_csv('dual_field_rsdm_results.csv', index=False)
        ...
        print(f"\nDual-Field Model-Free Fitting Results:")
